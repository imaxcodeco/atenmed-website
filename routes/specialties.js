...
router.post('/', [
    ...
    try {
        ...
        if (error.code === 11000) {
            return res.status(400).json({ success: false, error: 'Especialidade já existe para esta clínica' });
        }
...
router.put('/:id', [
    ...
    try {
        const updated = await Specialty.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!updated) return res.status(404).json({ success: false, error: 'Especialidade não encontrada' });
...
router.delete('/:id', [
    ...
    try {
        const specialty = await Specialty.findByIdAndUpdate(req.params.id, { active: false }, { new: true });
        if (!specialty) return res.status(404).json({ success: false, error: 'Especialidade não encontrada' });
