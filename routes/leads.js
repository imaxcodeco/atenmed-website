...
// Rate limiting para criação de leads (evitar spam)
const createLeadLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hora
  max: 20, // 20 leads por IP por hora
...
router.get(
  ...
  [
    ...
    query('status')
      .optional()
      .isIn(['novo', 'contato_feito', 'negociacao', 'proposta_enviada', 'fechado', 'perdido']),
...
router.delete(
  '/:id',
  [authenticateToken, authorize('admin'), param('id').isMongoId().withMessage('ID inválido')],
  validateRequest,
...
router.post(
  ...
  [
    ...
    param('id').isMongoId().withMessage('ID inválido'),
    body('tipo')
      .isIn(['contato', 'email', 'whatsapp', 'proposta', 'reuniao', 'outros'])
  ...
  async (req, res) => {
    try {
      ...
      await lead.adicionarHistorico(req.body.tipo, req.body.descricao, req.user.email);
...
router.patch(
  ...
  validateRequest,
  logActivity('update_lead_status'),
  async (req, res) => {
    try {
      const lead = await Lead.findById(req.params.id);
...
router.get(
  '/stats/overview',
  [authenticateToken, authorize('admin', 'vendedor')],
  logActivity('get_lead_stats'),
  async (req, res) => {
    try {
