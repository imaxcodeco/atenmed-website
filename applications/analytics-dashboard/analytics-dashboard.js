...
/* global Chart */ // Chart.js é carregado via CDN
...
async function loadData() {
  ...
  try {
    ...
    await Promise.all([loadKPIs(), loadCharts(), loadDoctorsPerformance()]);
...
async function loadDoctorsPerformance() {
  try {
    ...
    tbody.innerHTML = Object.values(byDoctor)
      .map((doctor) => {
        const attendanceRate =
          doctor.total > 0 ? ((doctor.completed / doctor.total) * 100).toFixed(1) : 0;
...
// ===== FUNÇÕES DE API =====
// eslint-disable-next-line no-unused-vars
async function fetchAppointments(_startDate, _endDate) {
  try {
    // Mock data temporário - TODO: Implementar endpoint GET ${API_BASE_URL}/appointments
...
async function fetchWaitlistStats() {
  try {
    // Mock data temporário - TODO: Implementar endpoint GET ${API_BASE_URL}/waitlist/stats/overview
    return {
      active: 15,
      converted: 8,
      conversionRate: '53.3%',
...
async function fetchReminderStats() {
  try {
    // Mock data temporário - TODO: Implementar endpoint GET ${API_BASE_URL}/confirmations/stats
    return {
      with24hReminder: 45,
      with1hReminder: 42,
      confirmed: 38,
