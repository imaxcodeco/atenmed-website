...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const clinicSlug = pathParts[pathParts.indexOf('clinica') + 1] || 
...
function renderClinicInfo() {
    ...
    document.getElementById('clinic-address').textContent = 
        clinic.address ? `${clinic.address.street}, ${clinic.address.number} - ${clinic.address.city}` : '-';
...
async function loadSpecialties() {
    try {
        const response = await fetch(`${API_BASE_URL}/appointments/clinics/${bookingApp.clinic._id}/specialties`);
        const data = await response.json();
...
function renderSpecialtiesSidebar() {
    ...
    container.innerHTML = bookingApp.specialties.map(spec => 
...
function renderDoctorsSidebar() {
    ...
    container.innerHTML = bookingApp.doctors.map(doctor => `
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (bookingApp.selectedDate && 
            bookingApp.selectedDate === date.toISOString().split('T')[0]) {
...
function renderTimeSlots() {
    ...
    if (bookingApp.availableSlots.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #64748b;">Nenhum horário disponível nesta data. Por favor, escolha outra data.</p>';
        return;
    }
    
    container.innerHTML = bookingApp.availableSlots.map(slot => `
...
function updateBookingSummary() {
    ...
    document.getElementById('summary-date').textContent = 
        new Date(bookingApp.booking.date).toLocaleDateString('pt-BR');
