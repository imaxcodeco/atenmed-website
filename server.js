...
const corsOptionsDelegate = (req, callback) => {
  ...
  logger.warn(
    `⚠️ Request bloqueado em produção. Path: ${req.path}, Method: ${req.method}, Host: ${host}, Referer: ${referer}, User-Agent: ${userAgent}`
  );
...
// Rate limiting global (EXCETO para webhooks do WhatsApp)
...
const authLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    logger.warn(`Rate limit excedido para ${req.ip} em ${req.path}`);
    res.status(429).json({
      success: false,
      error: 'Muitas tentativas de login. Tente novamente em 15 minutos.',
      retryAfter: 900,
...
const registrationLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    logger.warn(`Rate limit de cadastro excedido para ${req.ip} em ${req.path}`);
    res.status(429).json({
      success: false,
      error: 'Muitas tentativas de cadastro. Tente novamente em 1 hora.',
      retryAfter: 3600,
...
app.use(
  '/apps/cost-monitoring',
  express.static(path.join(__dirname, 'applications/cost-monitoring'))
);
...
app.use(
  '/apps/analytics',
  express.static(path.join(__dirname, 'applications/analytics-dashboard'))
);
...
app.get('/confirmar/:id', async (req, res) => {
  try {
    ...
    res.send(
      '<html><body style="font-family:Arial; text-align:center; padding:40px;">✅ Presença confirmada com sucesso.</body></html>'
    );
  } catch (e) {
...
app.get('/cancelar/:id', async (req, res) => {
  try {
    ...
    res.send(
      '<html><body style="font-family:Arial; text-align:center; padding:40px;">❌ Consulta cancelada com sucesso.</body></html>'
    );
  } catch (e) {
...
app.get(
  ['/politica-de-privacidade', '/privacidade', '/politica-de-privacidade.html'],
  (req, res) => {
...
const server = app.listen(PORT, () => {
  const BASE_URL =
    process.env.APP_URL ||
    (process.env.NODE_ENV === 'production' ? 'https://atenmed.com.br' : `http://localhost:${PORT}`);
